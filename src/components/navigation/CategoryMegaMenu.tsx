  ...
  return (
    ...
    >
      <button
        ...
        aria-expanded={open}
        className="flex items-center gap-1 text-sm font-medium text-foreground hover:text-brand-dark"
      >
        Categorieën
            ...
            {categories.map((category) => {
              ...
              return (
                <div key={category.slug}>
                  <Link
                    href={`/${category.slug}`}
                    className="text-sm font-semibold text-ink hover:text-brand-dark dark:text-foreground"
                  >
                    {category.title}
                  </Link>
                  {items.length > 0 ? (
