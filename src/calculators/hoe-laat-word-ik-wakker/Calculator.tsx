...
import { berekenWakkerWordTijden, formatTijdstip } from "@/lib/calculations/slaapcyclus";
...
export function HoeLaatWordIkWakkerCalculator() {
  const [uur, setUur] = useState("23");
  const [minuut, setMinuut] = useState("0");

  const wektijden = useMemo(
    () => berekenWakkerWordTijden({ uur: Number(uur), minuut: Number(minuut) }),
    [uur, minuut],
  );
  ...
  return (
    <div className="rounded-xl border border-border bg-surface p-6">
      <div className="grid gap-4 sm:grid-cols-2">
        <SelectField id="uur" label="Uur (naar bed gaan)" value={uur} onChange={setUur} options={UUR_OPTIES} />
        <SelectField id="minuut" label="Minuut" value={minuut} onChange={setMinuut} options={MINUUT_OPTIES} />
      </div>

      <div className="mt-6">
        <h3 className="text-sm font-medium text-foreground">Word rond deze tijden wakker</h3>
        <div className="mt-3 divide-y divide-border rounded-xl border border-border bg-surface">
          {wektijden.map(({ tijdstip, aantalCycli }) => (
